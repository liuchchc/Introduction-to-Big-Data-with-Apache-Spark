...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    sumValue = float(sum(IDandRatingsTuple[1]))
    lenValue = len(IDandRatingsTuple[1])
    return (IDandRatingsTuple[0], (lenValue, sumValue / lenValue))
...
def computeError(predictedRDD, actualRDD):
    ...
    #print "predictedReformattedRDD:", predictedReformattedRDD.collect()
    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda x:((x[0], x[1]), x[2]))

    #print "actualReformattedRDD:", actualReformattedRDD.collect()
    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                        .map(lambda x: (x[0], math.fabs(x[1][0] - x[1][1]) ** 2)))

    #print "squaredErrorsRDD:", squaredErrorsRDD.collect()
    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.map(lambda x: ("xxx", x[1])).reduceByKey(lambda x, y: x + y)
   
    # Count the number of entries for which you computed the total squared error
    numRatings = squaredErrorsRDD.count()

    # Using the total squared error and the number of entries, compute the RSME
    return math.sqrt(totalError.collect()[0][1] / numRatings)
...
# TODO: Replace <FILL IN> with appropriate code
print "trainingRDD::", trainingRDD.take(3)

trainingAvgRating = trainingRDD.map(lambda x: ("xx", x[2])).reduceByKey(lambda x, y: x + y).collect()[0][1] / trainingRDD.count()
...
myUnratedMoviesRDD = (moviesRDD
                      .map(lambda x: (myUserID, x[0])).subtract(myRatingsRDD))
