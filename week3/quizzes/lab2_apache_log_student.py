...
dayHostCount = dayGroupedHosts.map(lambda dh: (dh[0],len(dh[1])))
...
daysWithHosts = dailyHosts.map(lambda dh: dh[0]).collect()
hosts = dailyHosts.map(lambda dh: dh[1]).collect()
...
avgDailyReqPerHost = (sortedByDay
                      .map(lambda bd:(bd[0],len(bd[1]) / len(list(set(bd[1])))))).cache()
...
daysWithAvg = avgDailyReqPerHost.map(lambda dp: dp[0]).collect()
avgs = avgDailyReqPerHost.map(lambda dp: dp[1]).collect()
...
badEndpoints = badRecords.map(lambda bad: bad.endpoint)
...
badEndpointsCountPairTuple = badRecords.map(lambda bad: (bad.endpoint, 1))

badEndpointsSum = badEndpointsCountPairTuple.reduceByKey(lambda a, b : a + b)

badEndpointsTop20 = badEndpointsSum.takeOrdered(20, lambda bad: bad[1] * -1)
...
errHostsCountPairTuple = badRecords.map(lambda bad: (bad.host, 1))

errHostsSum = errHostsCountPairTuple.reduceByKey(lambda a, b : a + b)

errHostsTop25 = errHostsSum.takeOrdered(25, lambda bad: bad[1] * -1)
...
errDateCountPairTuple = badRecords.map(lambda bad: (bad.date_time.day, 1))

#print 'errDateCountPairTupley: %s' % errDateCountPairTuple.take(3)

errDateSum = errDateCountPairTuple.reduceByKey(lambda a, b: a + b)

#print 'errDateSum: %s' % errDateSum.take(20)

errDateSorted = errDateSum.sortByKey().cache()

#print 'errDateSorted: %s' % errDateSorted.take(20)
...
daysWithErrors404 = errDateSorted.map(lambda error: error[0]).collect()
errors404ByDay = errDateSorted.map(lambda error: error[1]).collect()
...
topErrDate = errDateSorted.takeOrdered(5, lambda error: error[1] * -1)
...
hourCountPairTuple = badRecords.map(lambda bad: (bad.date_time.hour, 1))
...
hoursWithErrors404 = hourRecordsSorted.map(lambda error: error[0]).collect()
errors404ByHours = hourRecordsSorted.map(lambda error: error[1]).collect()
