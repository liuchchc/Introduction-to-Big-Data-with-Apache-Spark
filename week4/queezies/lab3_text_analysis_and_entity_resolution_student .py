...
def simpleTokenize(string):
    ...
    return [i for i in (re.split(split_regex, string.lower())) if i != '']
...
def tokenize(string):
    ...
    """
    return [i for i in simpleTokenize(string) if i not in stopwords]
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda value: (value[0], tokenize(value[1])))
googleRecToToken = googleSmall.map(lambda value: (value[0], tokenize(value[1])))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.flatMap(lambda value: value[1]).count()
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.sortBy(lambda value: len(value[1]), False).collect()
...
def tf(tokens):
    ...
    for (k, v) in dicts.items():
        dicts[k] = dicts[k] / tokenCount
        
    #tokenMap = sc.parallelize(tokens).map(lambda value: (value, 1)).reduceByKey(lambda x,y : x + y).map(lambda value: (value[0], value[1] / tokenCount))
    return dicts
...
def idfs(corpus):
    ...
    uniqueTokens = corpus.map(lambda value: list(set(value[1])))
    
    tokenCountPairTuple = uniqueTokens.flatMap(lambda value: value).map(lambda value: (value, 1.0))
   
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a, b: a + b)
    return (tokenSumPairTuple.map(lambda value: (value[0], N / value[1])))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {k: float(v) * float(idfs[k]) for (k, v) in tfs.items()}
    return tfIdfDict
...
similarities = (crossSmall
                .map(lambda value: computeSimilarity(value))
                .cache())
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda value:(value[1] + ' ' + value[0], value[2]))

trueDupsRDD = (sims
               .join(goldStandard).map(lambda value:(value[0], value[1][0])))
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.map(lambda value: ("SIM", value[1])).reduceByKey(lambda a, b: a + b).collect()[0][1] / float(trueDupsCount)

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
avgSimNon = nonDupsRDD.map(lambda value: ("SIM", value[1])).reduceByKey(lambda a, b: a + b).collect()[0][1] / float(nonDupsRDD.count())
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda value: (value[0], tokenize(value[1])))
googleFullRecToToken = google.map(lambda value: (value[0], tokenize(value[1])))
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda value: (value[0], tfidf(value[1], idfsFullWeights)))
googleWeightsRDD = googleFullRecToToken.map(lambda value: (value[0], tfidf(value[1], idfsFullWeights)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda value:(value[0], norm(value[1]))).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda value:(value[0], norm(value[1]))).collectAsMap()
...
def invert(record):
    ...
    """
    recordKey = record[0]
    recordTokens = record[1]
    result = []
    for (k, v) in recordTokens.items():
        #result.append([(k, v),recordKey])
        result.append([k,recordKey])
    return result

amazonInvPairsRDD = (amazonWeightsRDD
                    .map(lambda value:invert(value)).flatMap(lambda value: value)
                    .cache())
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(lambda value: swap(value)).groupByKey()
                .cache())
...
def fastCosineSimilarity(record):
    ...
    for x in tokens:
        sumAll += amazonWeightsBroadcast.value[amazonRec][x] * googleWeightsBroadcast.value[googleRec][x]
       
    value = sumAll / (norm(amazonWeightsBroadcast.value[amazonRec]) * norm(googleWeightsBroadcast.value[googleRec]))
...
similaritiesFullRDD = (commonTokens
                       .map(lambda value: fastCosineSimilarity(value))
                       .cache())
